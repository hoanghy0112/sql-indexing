...
Full RAG workflow agent for answering database questions.
Handles: Retrieve → Enrich → Search Values → Generate with SQL retry logic.
"""
...
class ReasoningAgentState(TypedDict):
    """State for the reasoning agent workflow."""
...
async def retrieve_node(state: ReasoningAgentState) -> ReasoningAgentState:
...
async def enrich_node(state: ReasoningAgentState) -> ReasoningAgentState:
    """Get detailed table insights including column metadata."""
...
async def search_values_node(state: ReasoningAgentState) -> ReasoningAgentState:
    ...
    for term_info in searchable_terms:
        ...
        for table in table_insights:
            for column in table.get("columns", []):
                ...
                # Also check if column has categorical values
...
async def generate_node(state: ReasoningAgentState) -> ReasoningAgentState:
...
def should_retry_generate(state: ReasoningAgentState) -> str:
...
def create_reasoning_agent() -> StateGraph:
    """Create the reasoning agent workflow."""
    workflow = StateGraph(ReasoningAgentState)
...
async def run_reasoning_agent(
    ...
    """
    Run the reasoning agent to answer a database question.
    ...
    # Run the agent
    final_state = await reasoning_agent.ainvoke(initial_state)
